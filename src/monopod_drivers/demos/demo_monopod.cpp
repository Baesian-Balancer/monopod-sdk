...
/**
 * @brief Simple PD control on the monopod
 */
class MonopodController
{
private:
    /**
     * @brief is the leg to control
     */
    std::shared_ptr<blmc_drivers::Monopod> monopod_;
...
public:
     ...
     * @param analog_sensor
     */
    MonopodController(std::shared_ptr<blmc_drivers::Monopod> monopod)
        : monopod_(monopod)
    {
        stop_loop_ = false;
    }
...
private:
    ...
    void loop()
    {
        ...
        while (!stop_loop_)
        {
            // TODO: find a good position target
            double position_target = 1;

            double position_hip =
                monopod_[leg]->get_motor_measurement(blmc_drivers::Monopod::hip,
                                            blmc_drivers::Monopod::position)
                    ->newest_element();
            double velocity_hip =
                monopod_[leg]->get_motor_measurement(blmc_drivers::Monopod::hip,
                                            blmc_drivers::Monopod::velocity)
                    ->newest_element();

            double position_knee =
                monopod_[leg]->get_motor_measurement(blmc_drivers::Monopod::knee,
                                            blmc_drivers::Monopod::position)
                    ->newest_element();
            ...
            monopod_[leg]->set_current_target(current_target_knee,
                                     blmc_drivers::Leg::knee);
            monopod_[leg]->set_current_target(current_target_hip,
                                     blmc_drivers::Leg::hip);
            monopod_[leg]->send_if_input_changed();
...
int main(int, char**)
{
    ...
    auto can_bus_2 = std::make_shared<blmc_drivers::CanBus>("can2");
    auto can_bus_3 = std::make_shared<blmc_drivers::CanBus>("can1");

    // the board is used to communicate with the can bus, it takes one upon
    // creation
    auto motor_board = std::make_shared<blmc_drivers::CanBusMotorBoard>(can_bus_1);
    auto encoder_board = std::make_shared<blmc_drivers::CanBusMotorBoard>(can_bus_2);
    auto encoder_board_2 = std::make_shared<blmc_drivers::CanBusMotorBoard>(can_bus_3);
    // create motors and sensors ---------------------------------------------
    auto motor_hip = std::make_shared<blmc_drivers::Motor>(motor_board, 0);
    auto motor_knee = std::make_shared<blmc_drivers::Motor>(motor_board, 1);

    auto leg = std::make_shared<blmc_drivers::Leg>(motor_hip, motor_knee);
    rt_printf("leg is set up \n");

    auto encoder_bc = std::make_shared<blmc_drivers::Encoder(encoder_board,0);
    auto encoder_by = std::make_shared<blmc_drivers::Encoder(encoder_board,1);
    auto encoder_bp = std::make_shared<blmc_drivers::Encoder(encoder_board_2,0);
    
    auto planarizer = std::make_shared<blmc_drivers::Planarizer(encoder_bc, encoder_by, encoder_bp);
    rt_printf("planarizer is set up \n");

    auto monpod = std::make_shared<blmc_drivers::Monopod(leg, planarizer);

    MonopodController monopod_controller(monopod);
