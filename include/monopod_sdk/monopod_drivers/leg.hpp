...
public:
	...
	bool initialize()
	{
		...
		auto motor_hip_joint = std::make_shared<monopod_drivers::SafeMotor>(
			board_,
			hip_joint,
			motor_max_current_ * 0.99); // 0.99 means that safe motor will have a chance to limit current before

		auto motor_knee_joint = std::make_shared<monopod_drivers::SafeMotor>(
			board_,
			knee_joint,
			motor_max_current_ * 0.99);

		joints_[hip_joint] = std::make_shared<MotorJointModule>(motor_hip_joint,
		...
		joints_[knee_joint] = std::make_shared<MotorJointModule>(motor_knee_joint,
		                                                         0.025, //motor_constants[i],
		                                                         9.0, //gear_ratios[i],
		                                                         0.0, //zero_angles[i],
		                                                         false,
	 ...
	 *
	 * @param torque_targets are the torque to achieve on the motor card. the data format is {hip, knee}
	...
	/**
	 * @brief Calibrate the leg. See motor_joint_module.hpp for explanation of parameters
	 * and logic.
	 */
	bool calibrate(const double& hip_home_offset_rad, const double& knee_home_offset_rad)
	{
...
private:
	 ...
	 */
	std::unordered_map<JointNameIndexing, std::shared_ptr<monopod_drivers::MotorJointModule>> joints_;
...
private:
		...
		double profile_step_size_rad = 0.001)
	{
		...
		joints_[hip_joint]->init_homing((int)hip_joint,
		                                search_distance_limit_rad,
		                                home_offset_rad[0],
		                                profile_step_size_rad);

		joints_[knee_joint]->init_homing((int)knee_joint,
		                                 search_distance_limit_rad,
		                                 home_offset_rad[1],
		                                 profile_step_size_rad);
	 ...
	 */
	GoToReturnCode go_to(LVector angle_to_reach_rad, double average_speed_rad_per_sec = 1.0)
	{
		...
		do
		{
			...
			desired_torque = joints_[hip_joint]->execute_position_controller(desired_pose);
			joints_[hip_joint]->set_torque(desired_torque);

			desired_pose = min_jerk_trajs[1].compute(current_time);
			desired_torque = joints_[knee_joint]->execute_position_controller(desired_pose);
