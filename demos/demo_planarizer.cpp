...
static THREAD_FUNCTION_RETURN_TYPE printing_loop(void *planarizer_ptr) {
  ...
  monopod_drivers::Planarizer &planarizer =
      *(static_cast<monopod_drivers::Planarizer *>(planarizer_ptr));

  while (true) {
    auto data = planarizer.get_measurements();
    std::cout << "Current Position for: "
              << "(boom_connector_joint: "
              << data[monopod_drivers::boom_connector_joint]
                     [monopod_drivers::position]
              << ")"
              << "(planarizer_yaw_joint: "
              << data[monopod_drivers::planarizer_yaw_joint]
                     [monopod_drivers::position]
              << ")"
              << "(planarizer_pitch_joint: "
              << data[monopod_drivers::planarizer_pitch_joint]
                     [monopod_drivers::position]
              << ")" << std::endl;
    ...
    /*
     * You get the indexes from the common header files. you can use the map to
     * get them better.
     */
  }
...
int main(int, char **) {
  ...
  auto planarizer = std::make_shared<monopod_drivers::Planarizer>(board_);
  planarizer->initialize(2);
